...
def main():
    ...
    if st.session_state.logged_in:
        if st.session_state.user["role"] == 'admin':
            ...
            menu = ['Home', 'Add Product', 'Delete Product']  # 삭제 기능 추가
            ...
            elif choice == 'Delete Product':  # 삭제 기능 추가
                st.subheader('Delete a Product')
                name = st.text_input('Product Name to delete')
                if st.button('Delete'):
                    delete_product_response = requests.get('http://localhost:8000/delete_product', params={"name": name})
                    if delete_product_response.status_code == 200:
                        st.success(delete_product_response.json()["message"])

                    # 제품을 삭제한 후에 제품 목록을 다시 불러옴
                    products_response = requests.get('http://localhost:8000/products')
                    if products_response.status_code == 200:
                        products = products_response.json()
                        if products:
                            st.write("Here are the available products:")
                            st.write("| Name | Category | Price |")
                            st.write("| --- | --- | --- |")
                            for product in products:
                                st.write(f"| {product['name']} | {product['category']} | ${product['price']} |")
                                if 'thumbnail_url' in product and product['thumbnail_url'] != '':
